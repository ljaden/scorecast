      ...
      <div className="flex gap-3 justify-between items-center border-b border-gray-300 py-4 px-2 mb-4 bg-white shadow-2xl">
        ...
        <p>
          ...
          <span className="text-gray-500 text-sm">
            {rtkDate.todayFormatted === rtkDate.dateFormatted ? " (Today)" : ""}
          </span>
        </p>
        <AiOutlineCaretRight
          className="cursor-pointer"
      ...
      <ul className="flex justify-center gap-4 flex-wrap">
        {schedule ? (
          schedule?.games.map((game) => (
            <li
              key={game.gameId}
              className="w-48 my-3 pr-4 pl-4 pb-4 pt-1 border hover:bg-red-300 hover:border-black cursor-pointer whitespace-nowrap inline-block bg-white shadow-2xl"
